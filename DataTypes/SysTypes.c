...
SysBool sys_instance_create(SysTypeInstance *instance, SysType type) {
  sys_return_val_if_fail(instance != NULL, false);
  TypeNode *node;
  SysTypeClass *cls;
...
SysTypeInterface* sys_type_class_get_iface(TypeNode *node, SysType iface_type) {
  IFaceEntry* entry;
  SysTypeClass *cls;
...
void sys_type_imp_interface(SysType instance_type, SysType iface_type, const SysInterfaceInfo *info) {
  ...
  nmem = sys_malloc0_N(sizeof(IFaceEntry *) * (cls->n_ifaces + 1));
  if (cls->n_ifaces > 0) {
    /* ifaces = Inew + Iself */
