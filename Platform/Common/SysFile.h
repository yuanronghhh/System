...
SYS_API SysInt sys_fstat(FILE* fp, struct stat * state);
SYS_API SysInt sys_lstat(const SysChar *filename, struct stat * state);
...
SYS_API SysBool sys_file_is_dir (const SysChar  *filename);

SYS_API SysBool sys_file_state_is_dir (SysFileState *state);
SYS_API SysBool sys_file_state_get_by_filename(const SysChar *filename, SysFileState* state);
SYS_API SysChar *sys_file_read_link(const SysChar  *filename, SysError **error);
