  ...
  SysBool    rfc8536)
{
  ...
  if (!rfc8536 && sys_str_equal(name, "UTC") == 0)
  {
    *offset = 0;
    return true;
  }
...
copy_windows_systemtime(SYSTEMTIME* s_time, TimeZoneDate* tzdate)
{
  ...
  if (s_time->wYear)
  {
    tzdate->mday = (SysDateDay)s_time->wDay;
    tzdate->wday = 0;
  }
  else
  ...
  TIME_ZONE_INFORMATION* tzi)
{
  ...
  std_name = sys_utf16_to_utf8((SysUniChar2*)tzi->StandardName, -1, NULL, NULL, NULL);
  if (std_name == NULL)
    return false;

  dlt_name = sys_utf16_to_utf8((SysUniChar2*)tzi->DaylightName, -1, NULL, NULL, NULL);
  if (dlt_name == NULL)
  {
    sys_free(std_name);
    return false;
  ...
  strncpy(rule->std_name, std_name, SYS_TIME_ZONE_NAME_SIZE - 1);
  strncpy(rule->dlt_name, dlt_name, SYS_TIME_ZONE_NAME_SIZE - 1);
  ...
  TimeZoneRule** rules)
{
  ...
  *rules = NULL;
  ...
  subkey = sys_strconcat(reg_key, key_name, NULL);
...
sys_time_zone_new_identifier(const SysChar* identifier)
{
  ...
  if (identifier)
  {
    ...
    tz = sys_hash_table_lookup(time_zones, (SysChar *)identifier);
    if (tz)
    {
      sys_atomic_int_inc(&tz->ref_count);
      SYS_UNLOCK(time_zones);
  ...
  else
  {
    ...
    if (tz_default)
    {
      ...
      if (!(resolved_identifier == NULL && sys_str_equal(tz_default->name, "UTC")) &&
        sys_str_equal(tz_default->name, resolved_identifier) != 0)
      {
