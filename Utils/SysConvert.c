...
SysBool sys_get_filename_charsets (const SysChar ***filename_charsets) {
  ...
  if (!(cache->charset && strcmp (cache->charset, charset) == 0))
  {
    ...
    p = sys_env_get ("SYS_FILENAME_ENCODING");
    if (p != NULL && p[0] != '\0') {
    ...
    else if (sys_env_get ("SYS_BROKEN_FILENAMES") != NULL)
    {
...
static SysBool is_asciialphanum (SysUniChar c) {

  return c <= 0x7F && isalnum (c);
}

static SysBool is_asciialpha (SysUniChar c) {
...
SysChar * sys_filename_display_basename (const SysChar *filename) {
  SysChar *basename;
  SysChar *display_name;

  sys_return_val_if_fail (filename != NULL, NULL);

  basename = (SysChar *)sys_path_basename (filename);
 ...
 * Since: 2.6
 **/
SysChar * sys_filename_display_name (const SysChar *filename) {
  SysInt i;
  const SysChar **charsets;
