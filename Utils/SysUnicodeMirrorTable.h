...
#define PACKTAB_UINT8 SysUInt8
#define PACKTAB_UINT16 SysUInt16
#define PACKTAB_UINT32 SysUInt32

/*
  generated by packtab.c version 3

  use SYS_GET_MIRRORING_DELTA(key) to access your table

  assumed sizeof(SysInt16): 2
...
static const SysInt16 MirLev3[4*49] = {
...
#define SYS_GET_MIRRORING_DELTA(x) \
        ((x) >= 0x10000 ? 0 :  \
        MirLev3[((x) & 0x03) + \
        MirLev2[((x) >> 2 & 0x03) + \
        MirLev1[((x) >> 4 & 0x0f) + \
...
#define SYS_GET_MIRRORING(x) ((x) + SYS_GET_MIRRORING_DELTA(x))

/* End of generated sysmirroringtable.h */
