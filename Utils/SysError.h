...
#define sys_debug_N(format, ...) sys_log(SYS_ARGS(sys_debug, format) stdout, SYS_LOG_DEBUG, format, __VA_ARGS__)
#define sys_info_N(format, ...) sys_log(SYS_ARGS(sys_info, format) stdout, SYS_LOG_INFO, format, __VA_ARGS__)
#define sys_warning_N(format, ...) sys_warning(SYS_ARGS(sys_warning, format) format, __VA_ARGS__)
#define sys_abort_N(format, ...) sys_abort(SYS_ARGS(sys_error, format) format, __VA_ARGS__)
#define sys_exit_N(exitcode, format, ...) sys_exit(SYS_ARGS(sys_error, exitcode) exitcode, format, __VA_ARGS__)
#define sys_error_set_N(error, format, ...) sys_error_set(SYS_ARGS(sys_error_set, format) error, format, __VA_ARGS__)

#define sys_error_N(format, ...) \
do {                                                          \
  sys_error(SYS_ARGS(sys_error, format) format, __VA_ARGS__); \
} while(0)
